...
- **Why the competitors' raw predictions are published here.** `benchmarks/results/`
  ...
  are silent on generated output; **CSpred/UCBShift2** carries no licence file in its
  repository. Where a tool's terms do not address its output, publishing the
  predictions is a judgement made here in favour of a checkable comparison — the
  alternative is a benchmark nobody can verify. If any author would rather their
  tool's output were not mirrored, open an issue and it will be removed and replaced
  with the derived per-residue errors.
